...
set -x
# Build the test
./bin/M2-Planet --architecture amd64 -f test/test26/lisp.h \
...
hex2 -f test/common_amd64/ELF-amd64-debug.hex2 \
	...
	-o test/results/test26-amd64-binary \
	--exec_enable || exit 4
...
then
	...
	out=$(./test/results/test26-amd64-binary --version 2>&1 )
	[ 0 = $? ] || exit 5
	[ "$out" = "Slow_Lisp 0.1" ] || exit 6

	# Verify that the resulting file works
	out=$(./test/results/test26-amd64-binary --file test/test26/test.scm)
	[ "$out" = "42" ] || exit 7
fi
exit 0
