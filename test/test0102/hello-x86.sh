...
blood-elf \
	...
	--entry _start \
	-o test/test0102/M1-macro-footer.M1 \
	|| exit 2
...
hex2 \
	...
	-o test/results/test0102-x86-binary \
	--exec_enable \
	|| exit 4
...
then
	...
	out=$(./test/results/test0102-x86-binary --version 2>&1 )
	[ 0 = $? ] || exit 5
	[ "$out" = "M1 1.0.0" ] || exit 6
