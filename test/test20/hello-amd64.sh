...
hex2 -f test/common_amd64/ELF-amd64.hex2 -f test/test20/struct.hex2 --LittleEndian --architecture amd64 --BaseAddress 0x00600000 -o test/results/test20-amd64-binary --exec_enable || exit 3

# Ensure binary works if host machine supports test
if [ "$(get_machine ${GET_MACHINE_FLAGS})" = "amd64" ]
then
	# Verify that the compiled program returns the correct result
	out=$(./test/results/test20-amd64-binary 2>&1 )
	[ 40 = $? ] || exit 4
