...
hex2 -f test/common_amd64/ELF-amd64-debug.hex2 \
	 ...
	 -o test/results/test21-amd64-binary \
	 --exec_enable || exit 4
...
then
	...
	out=$(./test/results/test21-amd64-binary --version 2>&1 )
	[ 0 = $? ] || exit 5
	[ "$out" = "blood-elf 0.1
(Basically Launches Odd Object Dump ExecutabLe Files" ] || exit 6
	...
	./test/results/test21-amd64-binary -f test/test21/test.M1 -o test/test21/proof || exit 7
