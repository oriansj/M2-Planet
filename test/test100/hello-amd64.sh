...
hex2 -f test/common_amd64/ELF-amd64-debug.hex2 \
	...
	-o test/results/test100-amd64-binary --exec_enable || exit 4

# Ensure binary works if host machine supports test
if [ "$(get_machine ${GET_MACHINE_FLAGS})" = "amd64" ]
then
	# Verify that the resulting file works
	./test/results/test100-amd64-binary --architecture x86 \
	...
	[ ! -e bin/M2-Planet ] && mv test/results/test100-amd64-binary bin/M2-Planet
fi
exit 0
