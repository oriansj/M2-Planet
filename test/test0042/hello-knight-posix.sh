...
TMPDIR="test/test0042/tmp-knight-posix"
mkdir -p ${TMPDIR}
...
hex2 \
	...
	--architecture knight-posix \
	--base-address 0x0 \
	-o test/results/test0042-knight-posix-binary \
	|| exit 3
...
then
	...
	vm --POSIX-MODE --rom ./test/results/test0042-knight-posix-binary --memory 2M
	[ 0 = $? ] || exit 3

elif [ "$(get_machine ${GET_MACHINE_FLAGS})" = "knight" ]
then
	# Verify that the compiled program returns the correct result
	./test/results/test0042-knight-posix-binary
	[ 0 = $? ] || exit 3
fi
exit 0
