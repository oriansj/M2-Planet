...
hex2 -f test/test0020/struct.hex2 --BigEndian --architecture knight-native --BaseAddress 0x00 -o test/results/test0020-knight-native-binary --exec_enable || exit 3

# Ensure binary works if host machine supports test
if [ "$(get_machine ${GET_MACHINE_FLAGS})" = "knight*" ]
then
	# Verify that the compiled program returns the correct result
	out=$(./test/results/test0020-knight-native-binary 2>&1 )
	[ 20 = $? ] || exit 4
